...
> [!Note]
> 新建项目时 .NET Core 版本请选择 2.0，因为如果选择 1.x 后，接下来添加 “Cam.SmartContract.Framework” 时会失败。

然后右击 `Dependencies`，单击 `Add Packages...`，搜索 “Cam” ，选择 “Cam.SmartContract.Framework”，点击 `Add Package`。


程序开始添加 Cam 智能合约框架，该过程大约需要几分钟。
...
点击 `Build`，`Build All` 编译相应代码，编译成功后，会在 bin/Debug/netstandard2.0/ 目录下生成 .dll 文件，该 .dll 文件是 .NET IL 语言的文件，之后会用 Camn 将其二次编译为 .avm 智能合约文件。

示例里项目名为 test1，编译后会生成 test1.dll，之后会用到这个文件。

### 3. Cam-compiler

在 GitHub 上下载 [Cam-compiler](https://github.com/camchain/Cam-compiler) 项目，用 Visual Studio for Mac 打开该解决方案，编译其中的 Camn 项目

   > [!Note]
   > 首次编译 Camn 需要还原 NuGet 程序包，根据网络速度 该过程可能需要几十分钟至几小时不等，并且中途可能失败十余次，请耐心尝试。
   >
   > 本次测试过程中我使用  Visual Studio for Mac 还原项目，又用 Visual Studio Code 还原项目，又用 dotnet 命令行还原项目，总之失败 N 次后，终于还原成功了。历时几个小时。
   >
   > 如果实在无法还原 NuGet，可以在 Windows 上**发布** Camn 项目，然后将发布后的项目复制到 macOS 上。
   >
   > 在 Windows 上发布 macOS 版本的 Camn 前需要将 Camn.csproj 文件打开，将 \<RuntimeIdentifiers>win10-x64\</RuntimeIdentifiers> 更改为 \<RuntimeIdentifiers>osx.10.12-x64\</RuntimeIdentifiers>
   >
   > 详细 RID 可以参考 [.NET Core Runtime IDentifier (RID) catalog](https://docs.microsoft.com/en-us/dotnet/core/rid-catalog)

发布成功后，会在 bin\Release\PublishOutput 目录下生成 Camn.dll 文件，记住这个文件夹，之后会用到。
...
直接用 dotnet Camn.dll test1.dll 编译会提示如下错误：
...
```
Cam.Compiler.MSIL console app v2.0.1.0 
