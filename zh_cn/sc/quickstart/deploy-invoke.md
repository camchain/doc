...
本文将介绍如何使用 cam-wallet 在 Cam 区块链上部署和调用智能合约。本文中的操作步骤是通用的，适用于包括 cac20 代币在内的所有合约类型。
## 准备工作
在开始部署之前，请确认您已经完成以下工作：

- 已确认您的合约需要部署。相关信息请参阅 [什么需要部署](overview.md#智能合约的部署)。
- 已编译好一个 .avm 合约文件。
- 已安装 cam-wallet 并完成区块同步。相关信息请参阅 [使用 cam-wallet 客户端](../../node/walletmanual.md)。

## 使用 cam-wallet 部署智能合约

1. 在 cam-wallet 中点击 `高级` -> `部署合约`。
...
1. 创建好钱包后，在 cam-wallet 中点击鼠标右键，选择 `创建合约地址` -> `自定义`。
...
要调用已发布到区块链上的智能合约，您可以使用 cam-wallet 进行以下操作：

1. 在 cam-wallet 中，右键点击合约地址，选择 `查看合约`。
...
也可以使用 Cam-CLI 提供的 [invoke 方法](../../api/v2.0.1/api/invoke.md) 调用合约。