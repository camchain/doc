...
使用 C# 开发智能合约时，你无法使用 C# 的全部特性。会存在一些限制以及不支持的功能，这些限制的根源，来自于 CamVM 和 Dotnet IL 的差异性。

因为 CamVM 更加精简，所以我们只能将有限的 C# /dotnet 特性编译为 AVM 文件。
...
因为 CamVM 只有一种 Integer，底层实现是 BigInteger，比 C# 的范围要大，所以这些整数类型均可使用。
...
```c#
ulongtotal_Cam = 200;
BigInteger ico_Cam = 300;
BigInteger balance_Cam = total_Cam- ico_Cam;
ulongvalue = 150;
```
...
不完全支持，由于在 CamVM 层次，string 也是作为 bytearray 处理，和 C# 中的 string 是不同的，编译到 AVM 的 string 实际是它的 UTF8 编码的 bytearray，请勿使用任何 string 高级处理函数，仅将 string 作为一种特殊类型处理。
...
Byte[] 例外，因为 byte[] 是 CamVM 底层的特别类型。
...
Dictionary 功能可以用 Cam DOTNET DEVPACK 中的 MAP 替代。
...
```c#
private const ulongtotal_Cam = total_ico_usd / Cam_to_usd * Cam_decimals;
publicstatic BigInteger TotalIcoCam()=> total_Cam;
```

#### C# 委托和事件的支持

C# 委托可以定义，定义的委托有两个功能，都是 CamVM 的特别功能。
...
调用这个事件则会被 Cam C# 编译器理解为调用 Notify 方法，可参考 NEP5 的通知事件。
...
Cam C# 编译器要求一个智能合约有且只有一个名为 Main 的 public static 函数作为入口点。
...
分别对应 Cam 智能合约的通知与 NEP4。

#### 内置特性

如果你观察 Cam DEVPACK 会看到很多 extern 的外部函数，实际上他们并没有外部实现，只是不需要实现。他们由特性标记功能。
...
```c#
 [Syscall("Cam.Account.GetBalance")]
...
```c#
[OpCode(Cam.VM.OpCode.LEFT)]
 public extern static byte[] Take(byte[] good, int index);
```

#### NONEMIT 特性

执行一个具有 NonEMit 特性的函数，通常都是用来完成一些满足语法的转换，实际上在 CamVM 底层并不需要转换。
